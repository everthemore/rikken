...
def plot_convergence(history_file: str = "eval_history.json", save_path: str = "docs/convergence.png"):
    ...
    # Plot 1: Overall Win Rate Progression
    ax1 = axes[0, 0]
    ax1.plot(iters, overall_wr, marker="o", linewidth=2.5, color="#2563eb", label="Neural Win Rate")
    ax1.axhline(50, color="gray", linestyle="--", alpha=0.7, label="Baseline (50%)")
    ax1.set_title("1. Overall Win Rate vs Generation", fontsize=12, fontweight="bold")
    ax1.set_xlabel("Generation / Iteration")
    ax1.set_ylabel("Win Rate (%)")
    ax1.set_ylim(40, 100)
    ax1.legend()

    # Plot 2: Offense vs Defense Win Rate
    ax2 = axes[0, 1]
    ax2.plot(iters, decl_wr, marker="s", linewidth=2, color="#10b981", label="Offense (When Neural Declares)")
    ax2.plot(iters, def_wr, marker="^", linewidth=2, color="#f59e0b", label="Defense (When Neural Defends)")
    ax2.set_title("2. Offense vs Defense Win Rates", fontsize=12, fontweight="bold")
